...
game_background = pygame.image.load('game.jpg')
menu_background = pygame.image.load('bg.jpg')


def login(name):
    cur.execute("SELECT name, score, level FROM mygame")
    row = cur.fetchall()
    for rows in row:
        if name == rows[0]:
            print(
    ...
    for rows in row:
        if name == rows[0]:
            if result[1] >= int(rows[2]) and result[0] > int(rows[1]):
                cur.execute(
                    f"UPDATE mygame set score = {str(result[0])}, level = {str(result[1])} where name = '{name}'")
                con.commit()
                return True
    return False
...
if login(name):
    cur.execute(f'''INSERT INTO mygame (name, score, level) VALUES('{name}', 0, 0)''')
    con.commit()
...
def main_menu():
    ...
    while menu:
        ...
        screen.blit(render_menu, (560, 700))
...
while working:
    ...
    screen.blit(render_level, (1220, 5))
    ...
    if x < 0 or x > width - block or y < 0 or y > height - block or len(snake) > len(set(snake)) or snake[-1] in walls:
        while working:
            ...
            screen.blit(render_end, (520, 400))
            pygame.display.flip()
            result = [score, level]
            if record(name):
